...
export function SiteHeader({ variant = "dark", showBrandName = true, showBadges = true, brandHref = "/", showDashboardLink = false, dashboardHref = "/dashboard" }: SiteHeaderProps) {
  ...
  // Build dynamic nav items with Dashboard when authenticated
  const dynamicNavItems = showDashboardLink 
    ? [
        { href: "/", label: "Home" },
                ...
                <Link
                  ...
                  className={`group relative rounded-xl px-5 py-3.5 text-base font-medium transition-all duration-300 ${
                    item.label.includes("Dashboard")
                      ? isDark 
                        ? "bg-gradient-to-r from-[var(--brand-red)]/20 to-[var(--brand-red)]/10 border border-[var(--brand-red)]/50 hover:border-[var(--brand-red)]/80 hover:bg-gradient-to-r hover:from-[var(--brand-red)]/30 hover:to-[var(--brand-red)]/15 shadow-lg shadow-red-500/20 font-semibold" 
                        : "bg-red-50 hover:bg-red-100 border border-red-300 hover:border-red-400 font-semibold"
                      : isDark 
